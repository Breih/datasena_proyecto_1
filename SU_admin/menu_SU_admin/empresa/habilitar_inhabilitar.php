...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['actualizar_estado'])) {
    ...
    if (!empty($cc) && !empty($nuevo_estado)) {
        $stmt = $conexion->prepare("UPDATE empresa SET estado_habilitacion = ? WHERE numero_documento = ?");
        if ($stmt) {
...
if (isset($_GET['cc'])) {
    ...
    $stmt = $conexion->prepare(
        "SELECT tipo_documento, numero_documento AS cc, nickname, numero_telefono AS telefono, 
         correo_electronico AS correo, direccion, rol_id AS rol, actividad_economica, estado_habilitacion 
